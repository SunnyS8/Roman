...
export class SelfieTool implements Tool {
  name = "selfie";
  description =
    "Сгенерировать селфи с полным сохранением внешности из референсного фото. ВАЖНО: обязан вызвать этот tool когда пользователь просит селфи/фото/картинку/изображение/селфи тебя. НЕЛЬЗЯ просто описать словами или вставить [Selfie] — нужно реально сгенерировать через этот tool. Если просят изменить внешность (прическа, одежда) — передай это в context. Внешность (лицо, черты) сохраняется автоматически из референса.";
  parameters = [
