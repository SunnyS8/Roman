...
export class ImageGenTool implements Tool {
  name = "image_gen";
  description =
    "Сгенерировать изображение по текстовому описанию. Если нужно сохранить внешность персонажа (Романа) — используй параметр reference. ВАЖНО: обязан вызывать когда пользователь просит любую картинку/изображение/иллюстрацию. НЕ писать [Image] или [Картинка].";
  parameters = [
